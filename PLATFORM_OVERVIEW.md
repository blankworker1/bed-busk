...
- [ ] Wireframes (`patron.html`, and the patron cards on `index.html`) — reflect availability status and window instead of the hardcoded "Next available: Oct 3 – Oct 20" placeholder
- [ ] Wireframes (`patron.html`, and the patron cards on `index.html`) — add bed / plate & cutlery icon tags next to each patron's exchange-type badges
- [ ] `artists_template.csv` — add a discipline-tag field (music / circus skills / portraits/art / theatre / other, non-exclusive)
- [ ] Wireframes (`artist.html`) — add discipline tags next to the craft/format fields
- [ ] City Hub gallery (Wikimedia Commons-sourced, above the map) — not yet built
