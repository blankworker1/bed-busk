...
City/Town is the foundational content layer — every other object (patrons, pitches, Guide content, reviews) attaches to a City node. Country holds real content (the national Guide) but is a linked reference, not something users browse *through*. Region has no page at all — it's a label for filtering, not a stop on the way to a city.

**Navigation is search- and map-driven, not hierarchical.** The homepage is a world map (one pin per active city) with a search box underneath — type a city name and go straight there, or click a pin. No Country/Region pages to click through first. This scales by adding rows to a city list, not by adding new page templates per country or region.

```mermaid
flowchart TD
    A[Homepage: world map + city search] --> D[City/Town page]
    D --> E[Pitches]
    D --> F[Patrons]
    D --> G[Guide: city-level permits, etiquette, barter notes]
    D --> H[Community notes]
    G -.links to.-> B[Country Guide: national barter/busking law]
```

| Page | Contains |
| --- | --- |
| Homepage | World map with one pin per active city, and a city-name search box with autocomplete (shows region + country per match) |
| Country | National Guide content (barter legal status, general busking law framework) — linked from a city's own Guide, not browsed to directly |
