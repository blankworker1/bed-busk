...
Four working, standalone pages under [`wireframes/`](./wireframes) — plain HTML/CSS, no build step, no framework. Open `wireframes/index.html` directly in a browser, or serve the folder (e.g. `npx serve wireframes`) once it's on GitHub Pages.

| Page | Covers |
| --- | --- |
| `index.html` | City Hub for Bosa — hero, a live Leaflet/OpenStreetMap city map with pitch pins, Pitches, Patrons, Guide preview, Community notes |
